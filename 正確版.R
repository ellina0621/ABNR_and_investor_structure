...
price_filtered <- price_filtered %>%
  ...
  mutate(
    RET_CO = open / lag(close) - 1, 
    RET_OC = close / open - 1,       
    Night_Return_Dummy = ifelse(RET_CO > 0, 1, 0),  #add dummy
...
#######處理日內資料開收盤成交量########
# 開盤成交量 (9000000 ~ 9050000)
opening_volume <- filtered_intraday_data %>%
  filter(MthTime >= 9000000 & MthTime < 9050000) %>%
...
merged_data <- merged_data %>%
  mutate(
    Interaction_Term_1 = Night_Return_Dummy * Opening_Retail_Dummy * Closing_Institutional_Dummy
  )

# 建立迴歸模型
model <- lm(
  Intraday_Reversal_Dummy ~ Night_Return_Dummy + Opening_Retail_Dummy +  Closing_Institutional_Dummy +  Interaction_Term_1 + factor(StkNo) + factor(MthDate),
  data = merged_data
)
...
stargazer(
  ...
  covariate.labels = c(
    ...
    "Closing Institutional Sell Dummy",
    "Interaction_Term_1"
  ),
  add.lines = list(
